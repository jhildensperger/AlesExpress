...
- (void)scrollViewTapped:(UITapGestureRecognizer *)recognizer {
	if (recognizer.state == UIGestureRecognizerStateRecognized) {
		...
		__block NSInteger elementIndex = [self elementContainingPoint:tapLocation];
		if (elementIndex != -1) { // point not in element
            ...
            if (self.delegate && [self.delegate respondsToSelector:delegateCall]) {
                ...
                __weak V8HorizontalPickerView *blockself = self;
                dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds * NSEC_PER_SEC));
                dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
                    [blockself.delegate horizontalPickerView:blockself didSelectElementAtIndex:elementIndex];
                });
            }
		}
	}
